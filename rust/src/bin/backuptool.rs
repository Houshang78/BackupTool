...
fn main() -> Result<()> {
    let cli = Cli::parse();
    match cli.cmd {
        Cmd::Backup { sources, dest, set, workers, checksum, exclude, delete, dry_run, cipher } => {
            if sources.is_empty() {
            ...
            let opt = BackupOptions {
                ...
                use_checksum: checksum, excludes: exclude, prune: delete, dry_run, cipher, passphrase,
            };
            let bar = make_bar();
