...
tar -xzf backuptool-v1.2.1-linux-x86_64.tar.gz  # oder Windows-.zip entpacken
./backuptool --help
./backuptool-gui
```
...
### macOS (.pkg oder .app)
```bash
sudo installer -pkg backuptool-1.2.1.pkg -target /     # CLI nach /usr/local/bin
backuptool --help
```
...
- Installer: `backuptool-setup-1.2.1.exe` (Inno Setup) ausführen — bietet
