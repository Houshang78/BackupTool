...
tar -xzf backuptool-v1.2.0-linux-x86_64.tar.gz  # oder Windows-.zip entpacken
./backuptool --help
./backuptool-gui
```
...
### macOS (.pkg oder .app)
```bash
sudo installer -pkg backuptool-1.2.0.pkg -target /     # CLI nach /usr/local/bin
backuptool --help
```
...
- Installer: `backuptool-setup-1.2.0.exe` (Inno Setup) ausführen — bietet
