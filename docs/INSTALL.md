...
tar -xzf backuptool-v1.1.0-linux-x86_64.tar.gz  # or unzip the Windows .zip
./backuptool --help
./backuptool-gui
```
...
### macOS (.pkg or .app)
```bash
sudo installer -pkg backuptool-1.1.0.pkg -target /     # CLI into /usr/local/bin
backuptool --help
```
...
- Installer: run `backuptool-setup-1.1.0.exe` (Inno Setup). It offers a Start-menu
