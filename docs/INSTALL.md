...
tar -xzf backuptool-v1.2.1-linux-x86_64.tar.gz  # or unzip the Windows .zip
./backuptool --help
./backuptool-gui
```
...
### Linux (.deb)
```bash
sudo apt install ./backuptool_1.2.1_all.deb
backuptool --help          # CLI
...
### macOS (.pkg or .app)
```bash
sudo installer -pkg backuptool-1.2.1.pkg -target /     # CLI into /usr/local/bin
backuptool --help
```
...
- Installer: run `backuptool-setup-1.2.1.exe` (Inno Setup). It offers a Start-menu
